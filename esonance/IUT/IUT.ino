...
#define IR_back_left 5
#define IR_back_center 9
#define IR_back_right A0

static int sensorReading[8]={};
static int left[3]={};
const int noOfSensor=8;
int integralCapacity=50;
...
int isL()
{
 if((!sensorReading[0] && !sensorReading[1] && !sensorReading[2] && !sensorReading[3] && !sensorReading[4]) &&(!left[0] || !left[1] || !left[2]))
 return 1;
 else return 0;
}
...
void loop()
{
  ...
  //else if(/*isAllFrontIrsOn()&&!isAllIrsOn()||*/isL())
 // rotateHardLeft();
  //else if(isAllIrsOn())
  //{
  // if(positn=START)
